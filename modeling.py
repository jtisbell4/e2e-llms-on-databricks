...
registered_name = "models.default.llamav2_7b_chat_model_new" # Note that the UC model name follows the pattern <catalog_name>.<schema_name>.<model_name>, corresponding to the catalog, schema, and registered model name
...
loaded_model = mlflow.pyfunc.load_model(f"models:/{registered_name}/2")
...
model_version = result # 1  # the returned result of mlflow.register_model
endpoint_config = {
  "name": endpoint_name,
  "config": {
    "served_models": [{
